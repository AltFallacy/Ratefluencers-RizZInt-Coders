...
import { HeroSkeleton, KpiSkeletonRow, ChartSkeleton } from "@/components/shared/loading-skeleton";
import { useAuthenticity } from "@/hooks/use-api";
...
export default function AuthenticityPage() {
  const { data: d, isError, isFetching } = useAuthenticity();
  const isBackendOffline = isError;

  const compositionData = d
    ? [
        { name: "Real",       value: d.followerComposition.real,       color: "#10b981" },
        { name: "Suspicious", value: d.followerComposition.suspicious,  color: "#f59e0b" },
        { name: "Bot",        value: d.followerComposition.bot,         color: "#f43f5e" },
      ]
    : [];
  ...
  if (isFetching && !d) {
    return (
      <div>
        <PageHeader title="Authenticity Analysis" description="Fake follower detection, bot engagement, and audience quality scoring" />
        <HeroSkeleton />
        <KpiSkeletonRow />
    ...
    <div>
      ...
      <div className="mb-6 grid grid-cols-2 gap-4 xl:grid-cols-4">
        {[
          ...
          >
            ...
            <div className="mt-3">
              <AnimatedProgressBar
                label="" value={m.value} delay={0.3 + i * 0.06}
                colorClass={m.isRisk ? "bg-rose-500" : "bg-emerald-500"} showPercent={false}
              />
            </div>
          </motion.div>
        ))}
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2 mb-6">
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, delay: 0.3 }}>
          <ChartCard title="Follower Composition" subtitle="Breakdown of audience quality segments">
            <ResponsiveContainer width="100%" height={240}>
              <PieChart>
                <Pie
                  data={compositionData} cx="50%" cy="50%"
                ...
                <Legend wrapperStyle={{ fontSize: 12, color: "#a1a1aa" }} formatter={(value) => <span style={{ color: "#a1a1aa" }}>{value}</span>} />
              </PieChart>
            </ResponsiveContainer>
          </ChartCard>
        </motion.div>
